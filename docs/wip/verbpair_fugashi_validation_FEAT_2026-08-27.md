...
- [ ] 驗證器:讀音關、後項拒絕上線,CoreVerb 既有單元測試全綠(回歸)。
- [ ] VerbPair dry-run:script_id 1207(による)、347/8463/17168(〜まくる)、7660/35039(〜切れない)、10525(お腹が空く=すく)全數被拒,拒絕原因正確分類。
- [ ] 正常動詞對照:めくる、どく、済む、決まる、伝わる等抽樣動詞的 dry-run 數量與基線差異 ≤ ±10%。
- [ ] 同表層異讀卡(汚れる けがれる/よごれる、退く ×3)各自只拿到讀音相符的句子(抽樣人工確認 ≥ 10 句)。
- [ ] 「よる」擴展加回後,撚る 的命中為 0 或全為真撚る句(語料若無則 0 屬正常)。
- [ ] 執行總結報告輸出拒絕原因 × 動詞分佈。
- [ ] `extra_search_keywords.json` 新舊格式皆可讀(相容測試)。
