...
### 6.6 假名擴展全量調查(2026-08-27,驗證上線後的紅利收割)

方法:對全部母卡動詞逐一實測「表層 ES 搜尋」與「假名 ES 搜尋」各命中多少
**通過四關驗證**的句子;表層未吃飽(<20)且假名有真貨者為候選,52 個候選
按門檻(假名驗證通過 ≥2,或表層歸零且 ≥1)補入 28 個擴展,再逐一抽驗句子品質。

- **高價值收益**(語料以假名/不同送り仮名書寫,表層搜尋整個餓死):
  成る→なる(0→20 滿額)、止める→やめる(**0→20,順帶解決贏者全拿的
  餓死問題**——語料的やめる都寫假名,與寫漢字的とめる天然分流)、
  見付ける→みつける(0→20)/見付かる(0→15)、纏める→まとめる(0→17)、
  籠もる→こもる(0→14)、掛かる→かかる(+10)、無くなる/無くす、
  背ける、被る/被せる、揃う/揃える、零す、包まる/包む、緩む、解く(ほどく)等。
- **抽驗後移除 8 個誤命中重災區**(短假名關鍵字的斷詞器誤判,驗證器無法
  防禦):ひる/ちる/うれる(色情場景的擬態・口齒不清文字被 MeCab 誤解析
  成動詞)、こる(「ってこった」)、くだる(全是「くだらない」慣用)、
  わく(全是湧く,UniDic 統合到沸く但語意不符)、そる(慣用+使役混入)、
  わたる(結巴文字混入)。**教訓:三音以下的假名擴展必須逐句抽驗後才可上線**。
