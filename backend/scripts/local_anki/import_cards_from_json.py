...
Generic JSON card import CLI: batch-imports notes of any model into
Anki from a standardized JSON file, with field validation, automatic
model installation, nested-JSON serialization, and knowledge-graph
relation creation.
...
async def _extract_and_create_relations(
    ...
    """從 JSON 關聯資料建立知識圖譜關聯（含雙向關聯展開）。

    Create knowledge-graph relations from JSON relation data,
    expanding bidirectional entries into two directed relations.

    Args:
        relation_service: 關聯服務實例。Relation service instance.
        source_note_id: 來源卡片的 Anki note id。Anki note id of the source card.
        source_label: 來源卡片的顯示標籤。Display label of the source card.
        relations_data: JSON 中的關聯條目列表。List of relation entries from JSON.
    """
    ...
    if relations_to_create:
        try:
            ...
            logger.error(f"自動寫入關聯時發生業務邏輯錯誤: {e}")


async def main() -> None:
    """腳本進入點：讀取 JSON、驗證欄位並批次匯入卡片與關聯。

    Script entry point: read the JSON file, validate fields against
    the model definition, and batch-import cards plus their relations.
    """
    ...
    model_manager = AnkiModelManager(
        ...
        # 本檔位於 backend/scripts/local_anki/，parents[2] = backend/
        # （原 parent.parent 是搬入 local_anki/ 前的層數，會錯指到
        #  backend/scripts/app/anki_models 並自動建立空資料夾）
        model_dir=Path(__file__).resolve().parents[2] / "app" / "anki_models",
    )
        ...
        async with session_factory() as session:
            ...
            for index, note_data in enumerate(notes_data):
                ...
                if model_name not in installed_models:
                    ...
                    except (FileNotFoundError, AnkiConnectError, FluencyTidesError) as e:
