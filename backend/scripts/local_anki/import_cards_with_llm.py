...
Generic LLM card generation and import CLI: reads batched user
prompts from a standardized JSON file, calls the LLM through
CardService.generate_card, and imports the results into Anki.
...
async def main() -> None:
    """腳本進入點：讀取請求 JSON、逐筆呼叫 LLM 生成並匯入卡片。

    Script entry point: read the request JSON, call the LLM for each
    entry via CardService, and import the generated cards into Anki.
    """
