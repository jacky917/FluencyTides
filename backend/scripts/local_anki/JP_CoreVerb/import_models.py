"""匯入 JP_CoreVerb 系列筆記模型至本地 Anki。

Import the JP_CoreVerb note models into the local Anki instance from
the four-file asset set (.json / _front.html / _back.html /
_style.css) via AnkiConnect.
...
註：Context 卡共用通用模型 `JP_Context_Dark`，該模型由 JP_VerbPair 側的
import_models.py 管理，此處不重複匯入。

使用方式：
    python scripts/local_anki/JP_CoreVerb/import_models.py
"""
...
async def import_all_models(client: AnkiClient, backend_directory: str) -> None:
    """依序匯入所有 JP_CoreVerb 模型。

    Sequentially import all JP_CoreVerb models into Anki.
