...
Update the Telegram Bot deep-link info (TG_Bot field) on existing
Anki cards after TG_BOT_USERNAME changes in .env; notes without the
TG_Bot field are skipped automatically.
...
async def main() -> None:
    """腳本進入點：解析參數並批次更新指定牌組卡片的 TG_Bot 欄位。

    Script entry point: parse CLI args and batch-update the TG_Bot
    field of every note in the target deck, skipping notes without it.
    """
