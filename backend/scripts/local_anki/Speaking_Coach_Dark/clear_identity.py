...
.. note::
   本檔與 ``Speaking_Trilingual_Dark/clear_identity.py`` 是**刻意的重複**——
   兩者僅 ``JSONS_DIR`` 與訊息措辭不同。抽成共用工具需多一層目錄參數與
   共用的錯誤訊息措辭, 成本高於重複本身。**修改其中一支時記得同步另一支。**

   This file is a deliberate duplicate of the trilingual one; they differ only
   in ``JSONS_DIR`` and wording. Factoring it out would cost an extra directory
   parameter and shared message wording, which outweighs the duplication.
...
（見 ``docs/wip/speaking_coach_identity_FEAT_2026-08-11.md`` §3.3）。
...
1. 清除單一檔案全部卡片的身分（先預覽）:
   python scripts/local_anki/Speaking_Coach_Dark/clear_identity.py \\
       --name "面接（2026-06-07）" --dry-run

2. 只清特定一張卡（序號對齊匯入腳本診斷訊息中的 ``#N``）:
   python scripts/local_anki/Speaking_Coach_Dark/clear_identity.py \\
       --name "面接（2026-06-07）" --index 2

3. 清除整個 jsons/ 目錄（複製整包資料夾另開一套牌組時必做）:
   python scripts/local_anki/Speaking_Coach_Dark/clear_identity.py --all
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Speaking_Coach_Dark 專用: 清除卡片 JSON 的身分證 (cardId / noteId)"
    )
    target = parser.add_mutually_exclusive_group(required=True)
    target.add_argument(
        "--name",
        type=str,
        help="jsons 目錄下的 JSON 相對路徑 (不含 .json，子資料夾需一併給，如 '面接（2026-06-07）')",
    )
    target.add_argument(
