...
包含三個純函數優先的模組（設計依據各寫在模組 docstring）：

- ``candidate_validator``：fugashi token 級候選驗證器，tagger 注入式。
- ``diversity_selector``：搭配/活用形分桶與 zigzag 兩段式配額分配。
- ``funnel``：漏斗編排 ``run_selection_funnel``，
